...
request
request
echo "Killproxy: (Fourth request)"
curl -s -x localhost:8888 tinyproxy.stats | grep "Number of requests" -A1
echo "Tinyproxy: (Seventh request)"
curl -s -x ${IP}:8888 tinyproxy.stats | grep "Number of requests" -A1 | grep 7
